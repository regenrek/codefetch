...
export default async function initCommand(args: any = {}) {
  const cwd = process.cwd();

  // Check for non-interactive mode
  const useDefault = args.default || args.d;
  const setupType = useDefault
    ? "default"
    : await consola.prompt("Choose setup type:", {
        type: "select",
