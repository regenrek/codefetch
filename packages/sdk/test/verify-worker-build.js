...
console.log(`  Environment: ${worker.isCloudflareWorker ? 'Worker' : 'Node.js'}`);
console.log(`  Cache limit: ${worker.getCacheSizeLimit() / 1024 / 1024}MB`);

// 2. Constants
console.log(`  Valid encoders: ${Array.from(worker.VALID_ENCODERS).join(', ')}`);
...
worker.countTokens('test', 'cl100k').then(tokens => {
  ...
  console.error('  ✗ Token counting error:', err.message);
  process.exit(1);
});