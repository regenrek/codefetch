...
async function analyzeCodeForAI(targetPath = ".", task = "review") {
  ...
  try {
    ...
    console.log(
      `Selected ${selectedFiles.length} files (${currentTokens} tokens)\n`
    );
...
Please structure your response with clear sections and examples.`,
    ...
    console.log(
      `   - GPT-4: ${gpt4Path} (${await countTokens(gpt4Prompt, "cl100k")} tokens)`
    );
    console.log(
      `   - Claude: ${claudePath} (${await countTokens(claudePrompt, "cl100k")} tokens)`
    );
