...
function bumpVersion(
  ...
  if (type === "major" || type === "minor" || type === "patch") {
    ...
    throw new Error(
      `Invalid version bump type: ${type}. Use 'major', 'minor', 'patch', or a specific version like '1.2.3'.`
    );
  }
  ...
  if (isAlpha) {
    ...
    if (alphaVersion < 0) {
      throw new Error(
        `Cannot create alpha version from non-alpha version ${currentVersion} without bumping base version (major, minor, patch, or specific).`
      );
    }
...
async function publishPackage() {
  ...
  console.log(
    `✅ Successfully completed ${isAlpha ? "alpha" : ""} release v${newVersion}!`
  );
}
